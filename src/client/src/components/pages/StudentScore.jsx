...
export default function StudentScore() {
  ...
  const [currentSemester, setCurrentSemester] = useState(null); // New state to store the current semester object
  ...
  useEffect(() => {
    fetch('/semester/by-me')
      .then(res => res.json())
      .then(data => {
        // Ensure semesterID is a number for consistent comparison
        const formattedSemesters = (data.semesters || []).map(sem => ({
          ...sem,
          semesterID: Number(sem.semesterID) // Explicitly convert to number
        }));
        setSemesters(formattedSemesters);
        if (formattedSemesters.length > 0) {
          setSelectedSemester(String(formattedSemesters[0].semesterID)); // Store as string for select value
  ...
  useEffect(() => {
    ...
    // Ensure selectedSemester is parsed as a number for comparison
    ...
    if (!foundSemester) { // If no semester found, clear data and stop loading
        setLoading(false);
        setScore(0);
        setHistory([]);
        return;
